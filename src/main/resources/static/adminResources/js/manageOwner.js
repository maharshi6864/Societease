...
function changeValues(obj) {
  ...
  numberOfHouse.innerHTML =
    "<option selected disabled value='none'>Select Floor Number</option>";
...
function bringOwnersToform(numberOfHouse, floor, blockNumber) {
  ...
  htp.open(
    "GET",
    `http://localhost:8088/searchOwners/${floor}/${blockNumber}`,
    true
  );
  htp.send();
}
...
function floorChange(floor) {
  ...
  document.getElementById("submit-model").innerHTML = "Submit";
  floor.value !== "none"
    ? (document.getElementById("submit-model").style.display = "")
    : "";
...
function createOwnerForm(ownerList, numberOfHouse, floor) {
  ...
  for (let i = 0; i < numberOfHouse; i++) {
    var prevoiusHouse = document.getElementById(
      "house-numbers-ownwers"
    ).innerHTML;
    document.getElementById("house-numbers-ownwers").innerHTML =
      prevoiusHouse +
...
function bringOwnersToTable(floor) {
  ...
  setTimeout(function () {
    ...
    htp.open(
      "GET",
      `http://localhost:8088/searchOwners/${floorNumber}/${blockName}`,
      true
    );
    htp.send();
  }, 100);
...
function createTable(listOfOwners) {
  ...
  for (var index = 0; index < listOfOwners.length; index++) {
    ...
    newTR.insertCell(
      4
    ).innerHTML = ` <a class="ml-3" style="font-size: 14px" href="#" onclick="getMemberInfo(${listOfOwners[index].id})"   data-toggle="modal"
                            data-target="#exampleModal-member-info">
        <i class="mdi mdi-information-outline menu-icon"></i>
      </a>`;
    newTR.insertCell(
      5
    ).innerHTML = ` <a class="ml-3" style="font-size: 14px" href="#" onclick="getOwnerVehicleInfo(${listOfOwners[index].id},${listOfOwners[index].blockVo.id})"   data-toggle="modal"
                            data-target="#exampleModal-owner-parking">
        <i class="mdi mdi-parking menu-icon"></i>
      </a>`;
    newTR.insertCell(
      6
    ).innerHTML = ` <a class="ml-3" style="font-size: 14px" href="#" onclick="getOwnerMaintenanceInfo(${listOfOwners[index].id},${listOfOwners[index].blockVo.id})"   data-toggle="modal"
                            data-target="#exampleModal-owner-maintenance">
        <i class="mdi mdi-cash-multiple menu-icon"></i>
      </a>`;
    newTR.insertCell(
      7
    ).innerHTML = `<button onclick="editOwner(${listOfOwners[index].id})" class="btn btn-outline-info" style="padding: 1.6px 9.6px;font-size: 14px" data-toggle="modal"
...
function emptyAddOwnerForm() {
  ...
  document.getElementById("number-of-floors").innerHTML =
    " <option value=''>- -</option>";
...
function displayOwnerForFirst() {
  ...
  if (blockId != null) {
    ...
    if (selectTagValue[3] == undefined) {
      document.getElementById(
        "owner-table"
      ).innerHTML = `<tr><td colspan="6" class="text-center "><div class="d-flex justify-content-center align-items-center flex-column"><i class="mdi mdi-account-multiple" style="
 ...
 margin: 0px !important; font-size: 170px"></i><p  >No Owner Found For Selected Block and Floor.</p></div></td></tr>`;
  ...
  setTimeout(function () {
    var selectTagsFloor = document.getElementById(
      "number-of-floors-search"
    ).childNodes;
...
<div class="d-flex justify-content-center align-items-center flex-column" style="margin-top: 7% !important;"><i class="mdi mdi-account-multiple" style="color: #7571f9 !important; margin: 0px !important; font-size: 170px;line-height: 0.5"></i><p  >No Member Info For This Owner.</p></div>
  ...
  for (var index = 0; index < memberObj.length; index++) {
    ...
    newTR.insertCell(
      1
    ).innerHTML = `<img class="rounded " src="/documents/memberPhotos/${memberObj[index].fileName}" alt="image">`;
...
function getOwnerVehicleInfo(ownerId, blockId) {
  ...
  htp.open(
    "GET",
    `http://localhost:8088/admin/getVehicleDetails/${ownerId}`,
    true
  );
  htp.send();
}
...
function getParkingList(vehicleObjs, blockId) {
  ...
  htp.open(
    "POST",
    `http://localhost:8088/admin/getParkingSpaces/${blockId}`,
    true
  );
  htp.setRequestHeader("Content-Type", "application/json");
  htp.onreadystatechange = function () {
...
function createVehicleInfoTable(vehicleObjs, parkingAllotmentList) {
  ...
  for (var index = 0; index < vehicleObjs.length; index++) {
    ...
    newTR.insertCell(
      0
    ).innerHTML = `<input type="hidden" id="vehicle-id-${index}" value="${
      vehicleObjs[index].id
    }"><p>${1 + index}</p>`;
    newTR.insertCell(
      1
    ).innerHTML = `<h6>${vehicleObjs[index].vehicleNumberPlate}</h6>`;
    newTR.insertCell(2).innerHTML = `${vehicleObjs[index].vehicleRCNumber}`;
    if (vehicleObjs[index].vehicleType === "Two Wheeler") {
      newTR.insertCell(
        3
      ).innerHTML = ` <i class=" mdi mdi-motorbike ml-2" style="font-size: 20px !important;color: #7571f9 !important;"></i>`;
      newTR.insertCell(
        4
      ).innerHTML = `<select onchange="twoWheelerOptionChanged(this)" class="form-control" id="vehicle-allotment-${index}" name="two-wheeler-parking-names">
                                                    <option value="Parking Not Alloted" >Allocate Parking For Two Wheeler</option>
                                               </select>`;
    } else {
      newTR.insertCell(
        3
      ).innerHTML = ` <i class=" mdi mdi-car ml-2" style="font-size: 20px !important;color: #7571f9 !important;"></i>`;
      newTR.insertCell(
        4
      ).innerHTML = `<select onchange="fourWheelerOptionChanged(this)" class="form-control" id="vehicle-allotment-${index}" name="four-wheeler-parking-names">
                                                    <option value="Parking Not Alloted">Allocate Parking For Four Wheeler</option>
                                               </select>`;
    }
    newTR.insertCell(
      5
    ).innerHTML = `<button class="btn btn-outline-danger" onclick="deallocateParking('vehicle-allotment-${index}')" style="padding: 0.1rem 0.6rem !important;">Deallocate</button>`;
  }
...
function addParkingOptions(parkingAllotmentList, vehicleObjs) {
  console.log(parkingAllotmentList);
  var fourWheelerSelects = document.getElementsByName(
    "four-wheeler-parking-names"
  );
  ...
  var twoWheelerSelects = document.getElementsByName(
    "two-wheeler-parking-names"
  );
...
function allocateVehicle() {
  ...
  for (let i = 0; i < table.rows.length; i++) {
    ...
    vehicleVo.allocatedParkingSpace = document.getElementById(
      `vehicle-allotment-${i}`
    ).value;
    arr.push(vehicleVo);
  }
...
function fourWheelerOptionChanged(fourWheelerSelect) {
  let fourWheelerParkingSelects = document.getElementsByName(
    "four-wheeler-parking-names"
  );
...
function twoWheelerOptionChanged(twoWheelerSelect) {
  ...
  let keysList = Object.keys(takenParkingList);
  let twoWheelerParkingSelects = document.getElementsByName(
    "two-wheeler-parking-names"
  );
...
function createMaintenanceDetailTable(maintenancePaidDetailList) {
  ...
  for (let index = 0; index < monthList.length; index++) {
    ...
    if (listOFMaintenancePaidMonths.includes(monthList[index])) {
      ...
      newTR.insertCell(
        2
      ).innerHTML = ` <div class="badge badge-danger"  >Unpaid</div>`;
      newTR.insertCell(
        3
      ).innerHTML = `<button class="btn btn-outline-success" style="padding: 0.1rem 0.6rem" onclick="maintenancePaid(localStorage.getItem('ownerIdForMain'),'${monthList[index]}',document.getElementById('maintenance-year').value)">Paid</button>`;
    }
  }
}
...
function maintenancePaid(ownerId, month, year) {
  ...
  htp.open(
    "GET",
    `http://localhost:8088/maintenancePaid/${ownerId}/${year}/${month}`,
    true
  );
  htp.send();
}
...
function maintenanceUnpaid(ownerId, month, year) {
  ...
  htp.open(
    "GET",
    `http://localhost:8088/maintenanceUnpaid/${ownerId}/${year}/${month}`,
    true
  );
  htp.send();
}
